...
for _ in range(T := int(input())):
    ...
    for _ in range(int(input())):
        ...
        if inside(x1, y1, cx, cy, r) != inside(x2, y2, cx, cy, r):
            count += 1
    print(count)
...
#Solution
'''
주어진 (x1, y1)과 (x2, y2)가 하나는 원 내부, 하나는 원 외부에 있다면 최소 한 번 원을 통과해야함
주어진 좌표에 대하여 좌표가 중심이 (cx, cy)이고 반지름이 r인 원 내부에 있으면 True, 아니면 False를 반환하는 함수를 정의함
(x1, y1)과 (x2, y2)에 대하여 inside 함수가 다른 결과를 반환하면 count에 1을 더함
즉, 하나는 원 내부, 하나는 원 외부에 있다면 count가 증가함
'''
